/* binguOS, written by Yaseen Reza 19/06/2022 */
//-----------------------------------------------------------------------------
// DEPENDENCIES
//-----------------------------------------------------------------------------
/* Import built-ins */
...
const char splash[17] = "binguOS V1.5";
...
void setup() {
  ...
  Wire.begin();
  Wire.setClock(400000);  // Set communications speed because MPU6050 is picky
...
//    Serial.println(degrees(roll));
  ...
  if (
    ...
    if (tlast_SDwritten_ms + 1000 / SDlograte_Hz < tlast_cyclestart_ms) {
      ...
      datalog.print(data_GPS0.lat);
      datalog.print(", ");
      datalog.print("GPS_0 longitude[deg] ");
      datalog.print(data_GPS0.lng);
      datalog.print(", ");
...
//    // Display angle of sideslip
//    char displayaos_deg[4];
//    zeroPadFloat(displayaos_deg, sizeof(displayaos_deg), degrees(AoS_rad), 1);
//    lcd.setCursor(10, 0);
//    lcd.write(byte(1)); // Beta
//    lcd.print(displayaos_deg);
...
void getPitotPressure(sensordata_q &sd, int choice012)
{
  ...
  switch (choice012) {
    ...
    case 2:
      digitalWrite(pin_S2, HIGH);
      digitalWrite(pin_S3, HIGH); // This is the bad one
      break;
    default:
...
void updatePitotSolution(sensordata_q sd0, sensordata_q sd1, sensordata_q sd2, float &AoA_rad, float &AoS_rad, float &qtrue_Pa)
{
  ...
  Serial.print(sd0.q_Pa);
  Serial.print("\t");
  Serial.print(sd1.q_Pa);
  Serial.print("\t");
  Serial.print(sd2.q_Pa);
  Serial.println("\t");
 ...
 Serial.println("Kalman filtered!");
  ...
  Serial.println("Quadratic formula!");
